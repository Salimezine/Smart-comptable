...
export async function updateMemberRole(
  ...
  await ensureAdmin(userId, companyId);
...
export async function createInvite(
  ...
  if (pendingInvite) {
    throw new AppError(409, "CONFLICT", "An invite is already pending for this email");
  }
...
export async function acceptInvite(token: string, userId: string) {
  ...
  if (normalizeEmail(user.email) !== normalizeEmail(invite.email)) {
    throw new AppError(403, "FORBIDDEN", "This invite was issued for a different email");
  }
...
async function isCompanyOwner(userId: string, companyId: string) {
  const company = await db
    .selectFrom("companies")
    .select("id")
    .where("id", "=", companyId)
...
async function ensureAdmin(userId: string, companyId: string) {
  ...
  if (!membership || membership.role !== "admin") {
    throw new AppError(403, "FORBIDDEN", "Only admins can perform this action");
  }
}
