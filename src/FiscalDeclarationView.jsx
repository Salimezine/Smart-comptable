...
function DataScrapedView() {
  ...
  const isFresh = daysSinceScrape !== null && daysSinceScrape <= 1;

  const freshnessConfig = daysSinceScrape === null
    ? { color: 'bg-slate-500', label: 'Inconnue', pulse: false }
    : daysSinceScrape === 0
    ? { color: 'bg-emerald-500', label: 'Aujourd\'hui', pulse: true }
    : daysSinceScrape === 1
    ? { color: 'bg-emerald-400', label: 'Hier', pulse: true }
    : daysSinceScrape <= 7
    ? { color: 'bg-amber-500', label: `Il y a ${daysSinceScrape} jours`, pulse: false }
    : { color: 'bg-rose-500', label: `Il y a ${daysSinceScrape} jours`, pulse: false };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <p className="text-[10px] text-slate-500 font-medium">Sources disponibles ({sources.length})</p>
          {isFresh && (
        ...
        {lastScraped && (
          <span className="flex items-center gap-1.5 text-[9px] font-semibold text-slate-400">
            <span className={`w-1.5 h-1.5 rounded-full ${freshnessConfig.color} ${freshnessConfig.pulse ? 'animate-pulse-dot' : ''}`} />
      ...
      {sources.length > 0 && (
        <div className="flex items-center gap-2 text-[9px] text-slate-600">
          ...
          <span>Dernière mise à jour: {lastScraped ? new Date(lastScraped).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}</span>
        </div>
      )}

      {sources.map((src, i) => (
        <div key={i} className="bg-slate-950/40 border border-slate-800/80 rounded-2xl overflow-hidden transition-all duration-200 hover:border-slate-700/60">
          <button
